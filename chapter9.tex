...
\begin{definition}\index{danweigen@单位根 (root of unity)}
	取定代数闭包 $\overline{F}|F$ 和 $n \in \Z_{\geq 1}$. 元素 $\zeta \in \overline{F}^\times$ 如满足 $\zeta^n = 1$ 则称为 $n$ 次\emph{单位根}, 如 $\zeta^d = 1 \iff d \mid n$ 则称 $\zeta$ 为 $n$ 次\emph{本原单位根}.\index{danweigen!本原 (primitive)}
\end{definition}
