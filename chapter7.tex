...
\end{tikzcd}\]
交换, 其中 $\mu_A$, $\mu_B$ 表代数的乘法, 并在图中省去结合约束. 下块的交换性可归结于 $c_\epsilon(-,-)$ 的函子性. 上块交换性从辫子观点则一目了然 (回忆 \S\ref{sec:braiding}):
...
\begin{Exercises}
	...
	\item 试证 Cartan 引理如次: 设 $V$ 为域 $F$ 上的有限维向量空间, $v_1, \ldots, v_m \in V$ 线性无关. 证明若 $w_1, \ldots, w_m \in V$ 满足于 $\sum_{i=1}^m v_i \wedge w_i = 0$, 则存在一族 $\left( a_{ij} \in F \right)_{1 \leq i,j \leq m}$ 使得 $w_i = \sum_{j=1}^m a_{ij} v_j$, 而且 $a_{ij} = a_{ji}$.
