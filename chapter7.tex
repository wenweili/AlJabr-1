...
任意环 $S$ 上的一个左模 $N$ 称为是忠实的, 如果 $\text{ann}(M) = \{0\}$; 换言之 $s \in S$ 满足于 $sN=\{0\}$ 当且仅当 $s=0$. 右模亦同. 代数 $A$ 透过左乘成为忠实 $R[x]$-模, 事实上 $A$ 的任何包含 $1$ 的子模皆是忠实的.
...
\begin{remark}\label{rem:algebra-otimes-zero}
	非零 $R$-代数的张量积可以是零代数. 例如取 $a, b \in \Z_{>1}$ 互素, 则存在整数 $u,v$ 使得 $1 = ua+vb$, 因而 $\Z/a\Z \dotimes{\Z} \Z/b\Z = 0$. 然而当 $A$, $B$ 都是非零自由 $R$-模时, 推论 \ref{prop:module-tensor-free} 确保 $A \dotimes{R} B$ 自由且秩为 $A$, $B$ 的秩之和, 因而非零. 当 $R$ 是域时, 以上假设总是成立. 这套论证可以直接推广到稍后要讨论的无穷张量积 (定义--定理 \ref{def:inf-tensor-product}).
\end{remark}
...
\begin{example}
	...
	\begin{gather}\label{eqn:matrix-tensor-3}
		M_n(A) \otimes M_n(B) \simeq A \otimes M_n(R) \otimes M_m(R) \otimes \simeq M_{nm}(A \otimes B).
	\end{gather}
...
\begin{itemize}
	...
	\item 光滑微分流形 $X$ 上的所有 $\CC$-值微分形式对运算 $(\omega, \eta) \mapsto \omega \wedge \eta$ 构成 $\CC$-代数, 记作 $A(X) = \bigoplus_{k=0}^{\dim X} A^k(M)$, 其中 $A^k(M)$ 表示 $k$ 次微分形式构成的 $\CC$-向量空间, 基本理论可参考 \cite[\S 3.2]{ChCh}. 回忆到 $A^0(X) = C^\infty(X)$ 而 $A^k(X)$ 中任意元素 $\omega$ 在局部坐标 $x_1, \ldots, x_n$ 下可唯一地表作
...
\begin{definition}[张量代数]\index[sym1]{$T(M)$, $T^n(M)$}\index{zhangliangdaishu@张量代数 (tensor algebra)}
	定义 $R$-模 $M$ 的\emph{张量代数}为 $T(M) := \bigoplus_{n=0}^\infty T^i(M)$, 其乘法和幺元分别由诸 $\mu_{i,j}$ 和 $R = T^0(M) \hookrightarrow T(M)$ 给出. 它带有自然的 $R$-模单同态 $M = T^1(M) \hookrightarrow T(M)$. 同时 $T(M)$ 也是定义 \ref{def:graded-module-alg} 中的分次代数, 其 $k$ 次齐次部分无非是 $T^k(M)$.
\end{definition}
...
\begin{theorem}\label{prop:tensor-alg-universal}
	张量代数满足如下泛性质: 对任意 $R$-代数 $A$ 连同 $R$-模同态 $f: M \to A$, 存在唯一的 $R$-模同态 $\varphi: T(M) \to A$ 使下图交换:
	\[\begin{tikzcd}[row sep=tiny]
...
\begin{theorem}\label{prop:Sym-wedge-universal}
	...
	\end{align*}
	其中 $U$ 表示忘却函子 $R\dcate{CAlg}_\Z, R\dcate{CAlg}^-_\Z \to R\dcate{Mod}$.
\end{theorem}
\begin{proof}
	以对称代数的情形为例, 上示同构映 $f: \Sym(M) \to A$ 为合成 $M \hookrightarrow \Sym(M) \xrightarrow{f} A$, 反向则映 $R$-模同态 $\varphi: M \to A$ 为 $f: \Sym(M) \to A$, $f(x_1 \cdots x_n) = \varphi(x_1) \cdots \varphi(x_n)$, 后者良定是命题 \ref{prop:Symm-Alt-univ} 的推论.
\end{proof}
...
\begin{gather*}
	T^n_\chi := \{x \in T(M): \forall \sigma \in \mathfrak{S}_n,\; \sigma x = \chi(\sigma)x\}, \\
...
\end{align*}
实际上仅取 $\tau= (i \quad i+1)$ 即可生成; 留意到上式对 $n=1$ 是平凡的. 另一方面, $1 - e_\chi = \frac{1}{n!} \sum_\tau (1 - \chi(\tau)^{-1} \tau)$ 的像即 $\Ker(e_\chi)$ 又包含于 $I^n_{\cdots}(M)$ (取 $\chi = 1, \sigma$, 此时 $\chi=\chi^{-1}$). 于是我们抵达以下结果.
\begin{theorem}
	当 $n! \in R^\times$ 时 $\Ker(e_1) = I^n_{\Sym}$, $\Ker(e_{\sgn}) = I^n_{\bigwedge}$, 因而有 $R$-模的同构
	\begin{gather*}
		e_1: T^n_{\Sym}(M) \longrightiso \Sym^n(M), \\
		e_{\sgn}: T^n_\wedge(M) \longrightiso \bigwedge^n(M).
	\end{gather*}
\end{theorem}
...
\begin{proof}
	...
	\[ u_1 \wedge \cdots \wedge u_{i_a} \wedge w_{i_{a+1}} \wedge \cdots \wedge w_{i_k} \]
	的系数非零, 那么用 $\Xi := \check{u}_2 \wedge \cdots \wedge \check{w}_{i_k}$ (取对偶基) 对 $\Lambda$ 缩并, 从 \eqref{eqn:contraction-concrete} 知结果是 $u_1$ 的非零倍数, 故 $W \not\subset W_0$. 反过来说, 假若 $\Lambda \in \bigwedge^k W_0$, 那么它无论如何缩并都仍在 $\bigwedge^{\leq k} W_0$ 中, 故 $W \subset W_0$. 断言的最后一条性质归因于 $\dim W = k \implies \dim \bigwedge^k W = 1$.
\end{proof}
...
类似于向量空间的情形, 无须选基也能对 $\varphi \in \End_R(M)$ 定义下述对象:
\begin{itemize}
	\item 行列式 $\det(\varphi) \in R$: 业已于定义 \ref{def:general-determinant} 探讨;
	\item 迹 $\Tr(\varphi) \in R$: 它无非是 $\varphi$ 在 \eqref{eqn:End-free-otimes} 下按模同态 $\End_R(M) \rightiso E^\vee \otimes E \to R$ 得到的像; 从矩阵观点看就是寻常的 $\Tr(A) = \sum_{i=1}^n a_{ii}$. \index[sym1]{Tr@$\Tr$}\index{ji-trace@迹 (trace)}
	\item 特征多项式 $\mathrm{char}(\varphi, X) := \det(X \cdot \identity - \varphi) \in R[X]$, 以 $X$ 为变元; 这里将 $\varphi$ 等同于 $\End_{R[X]}(M \otimes_R R[X])$ 中相应的元素.
\end{itemize}
